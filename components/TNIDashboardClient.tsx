...
export default function TNIDashboardClient({
    ...
    const [isActiveTNIOpen, setIsActiveTNIOpen] = useState(true);
    ...
    const getStatusText = (nom: any) => {
        if (nom.status === 'Approved' || (nom.status === 'Batched' && nom.managerApprovalStatus === 'Approved')) {
            return nom.status === 'Batched' ? 'Scheduled' : 'Approved';
        }
    ...
    const getStatusClass = (nom: any) => {
        if (nom.status === 'Batched' && nom.managerApprovalStatus === 'Approved') {
    ...
    return (
        <div className="space-y-10">
            ...
            {/* 2. Active Nominations Table */}
            <div className="space-y-3 bg-white p-4 rounded-3xl border border-slate-200 shadow-sm">
                <div
                    className="flex justify-between items-center cursor-pointer select-none pb-2"
                    onClick={() => setIsActiveTNIOpen(!isActiveTNIOpen)}
                >
                    <h3 className="text-base sm:text-lg font-bold text-slate-800 uppercase tracking-tight flex items-center gap-2">
                        <HiOutlineClipboardDocumentList className="text-blue-600 shrink-0 text-lg sm:text-xl" />
                        Active Training Needs
                        {!isActiveTNIOpen && (
                    ...
                    <button className="p-1 rounded-full hover:bg-slate-100 text-slate-500 transition-colors">
                        {isActiveTNIOpen ? <HiChevronUp size={20} /> : <HiChevronDown size={20} />}
                    </button>
                </div>
                ...
                {isActiveTNIOpen && (
                    <div className="pt-2 animate-in fade-in slide-in-from-top-2 duration-200">
                        {activeNominations.length === 0 ? (
                            <div className="p-8 border border-slate-200 rounded-3xl text-center text-xs text-slate-500 bg-slate-50 shadow-sm font-medium">
                                No active training needs found.
                            </div>
                        ) : (
                            <div className="overflow-x-auto border border-slate-200 rounded-2xl bg-white shadow-sm">
                                <table className="w-full text-xs text-left border-collapse">
                                    <thead>
                                        <tr className="bg-slate-50 border-b border-slate-200">
                                            ...
                                            <th className="px-3 sm:px-4 py-2 sm:py-3 font-bold text-slate-500 uppercase tracking-wider border-r border-slate-200 whitespace-nowrap">TNI Approval by Manager</th>
                                            <th className="px-3 sm:px-4 py-2 sm:py-3 font-bold text-slate-500 uppercase tracking-wider border-r border-slate-200 whitespace-nowrap">Program Status</th>
                                    ...
                                    <tbody className="divide-y divide-slate-200">
                                        {activeNominations.map((nom: any, idx: number) => (
                                            <tr key={nom.id} className="hover:bg-slate-50/50 transition-colors">
                                                ...
                                                <td className="px-3 sm:px-4 py-1.5 sm:py-3 border-r border-slate-200 whitespace-nowrap">
                                                    {(() => {
                                                        ...
                                                        const badgeClass = isRejected ? 'text-rose-600 bg-rose-50 border border-rose-100 rounded-xl' : (label === 'Pending' ? 'text-amber-600 bg-amber-50 border border-amber-100 rounded-xl' : 'text-emerald-600 bg-emerald-50 border border-emerald-100 rounded-xl');
                                                        return (
                                                ...
                                                <td className="px-3 sm:px-4 py-1.5 sm:py-3 border-r border-slate-200 whitespace-nowrap">
                                                    <div className="flex flex-col gap-0.5">
                                                        ...
                                                        {getStatusText(nom) === 'Scheduled' && (nom.batch?.trainingSession?.startDate || nom.batch?.proposedStartDate) && (
                                                            <span className="text-[9px] text-slate-500 font-medium block mt-1">
                                                                On: {new Date(nom.batch.trainingSession?.startDate || nom.batch.proposedStartDate).toLocaleDateString(undefined, { day: 'numeric', month: 'short', year: 'numeric' })}
                                                            </span>
                                                        )}
                                                    </div>
                                                </td>
