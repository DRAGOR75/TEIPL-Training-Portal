    ...
    try {
        ...
        const trainingSession = await db.trainingSession.create({
            data: {
                ...
                startDate,
                endDate,
                trainingDays: formData.get('trainingDays') ? parseFloat(formData.get('trainingDays') as string) : null,
                trainingHours: formData.get('trainingHours') ? parseFloat(formData.get('trainingHours') as string) : null,
                location: formData.get('location') as string,
                region: formData.get('region') as string,
                trainingLocationAddress: formData.get('trainingLocationAddress') as string,
...
export async function getPendingNominationsForProgram(programId: string) {
    const nominations = await db.nomination.findMany({
        where: {
            programId,
            status: { in: ['Pending', 'Approved', 'Absent'] },
