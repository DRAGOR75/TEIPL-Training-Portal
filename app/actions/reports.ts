    ...
    try {
        ...
        const [
            ...
            db.employee.groupBy({
                by: ['sectionName'],
                where: { 
                    nominations: { some: { status: { in: ['Pending', 'Approved', 'Absent'] } } },
                    ...(site ? { location: site } : {})
                },
                _count: { id: true }
            }),
            ...
            db.employee.groupBy({
                by: ['location'],
                where: { nominations: { some: { status: { in: ['Pending', 'Approved', 'Absent'] } } } },
                _count: { id: true }
            }),
            // 5. Grade-wise Demand
            db.employee.groupBy({
...
export async function getProgramParticipantDepth(programId: string) {
    ...
    return await db.nomination.findMany({
        where: { programId, status: { in: ['Pending', 'Approved', 'Absent'] } },
        include: {
            employee: {
                select: {
                    id: true,
...
export async function getFilteredParticipantDepth(filters: { programId?: string; site?: string }) {
    ...
    const where: any = { status: { in: ['Pending', 'Approved', 'Absent'] } };
...
export async function getUntrainedPendingTrainees() {
    ...
    const rawData = await db.$queryRaw<any[]>`
        ...
        WHERE n.status IN ('Pending', 'Approved', 'Absent')
        AND NOT EXISTS (
